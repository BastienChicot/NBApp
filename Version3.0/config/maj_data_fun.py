...
def miseajour():
    ...
    racine=tk.Tk()
    racine.geometry("500x300")
    racine.iconbitmap('config/icone.ico')
    racine.title("Mise à jour data")
    prop = tk.Label(racine, text="Mise à jour des données de l'application")
    progress=ttk.Progressbar(racine,orient="horizontal",length=100,mode='determinate')
    lab = tk.Label(racine, text="")
    ...
    def maj_data_fun():
        ...
        df2=[]
        progress['value']=5
        lab.config(text="Lancement du programme")
        racine.update_idletasks()
        ### RECUPERATION DES STATS PAR JOUEURS
        for code in code_list :
            progress['value']+=(1/len(code_list)*40)
            lab.config(text="Récupération des données \n (cette opération peut être longue, \n ne pas tenter de fermer la fenêtre)")
            racine.update_idletasks()
        ...
        progress['value']=50
        lab.config(text="Récupération des données sur les coachs")
        racine.update_idletasks()
        ...
        df_propre = []
        progress['value']=60
        lab.config(text="Nettoyage des données")
        racine.update_idletasks()
        ...
        base.to_csv('data/player_stat_'+str(date)+'.csv', sep=";") 
        ...
        progress['value']=75
        lab.config(text="Calcul des indicateurs")
        racine.update_idletasks()
        ...
        progress['value']=85
        lab.config(text="Construction du fichier final")
        racine.update_idletasks()
        x = df_coach.values
        ...
        progress['value']=95
        lab.config(text="Export du fichier final (nom : data_ML.csv")
        racine.update_idletasks()
        ...
        progress['value']=100
        lab.config(text="Traitement terminé")
    
    prop.pack()
    progress.pack()
    lab.pack()
    bt=tk.Button(racine,text='Lancer le téléchargement',command=maj_data_fun).pack()
    racine.mainloop()